...
namespace RpaWinUiComponentsPackage.AdvancedWinUiDataGrid.Models
{
    ...
    internal class CellRange
    {
        ...
        /// <summary>
        /// Vytvorí range z selection
        /// </summary>
        public static CellRange FromSelection(List<CellSelection> selection)
        {
            if (!selection.Any())
                return new CellRange(0, 0, 0, 0);

            var minRow = selection.Min(s => s.RowIndex);
            var maxRow = selection.Max(s => s.RowIndex);
            var minCol = selection.Min(s => s.ColumnIndex);
            var maxCol = selection.Max(s => s.ColumnIndex);
